...
(defpackage #:nvlime-sbcl
  (:use #:cl
        #:aio-sbcl
        #:nvlime-protocol
        #:nvlime-connection))

(in-package #:nvlime-sbcl)
...
(defmethod start-server ((backend (eql :sbcl)) host port swank-host swank-port dont-close)
  (vom:config t :info)
  (unless nvlime-connection:*connections*
    (setf nvlime-connection:*connections* (make-hash-table)))
  ...
  (unless nvlime-sbcl::*read-buffer-map*
    (setf nvlime-sbcl::*read-buffer-map* (make-hash-table)))

  (let* ((server (aio-sbcl:tcp-server
                   host port
                   :client-read-cb #'(lambda (afd data)
                                       (nvlime-sbcl::client-read-cb
                                         afd data
                                         swank-host swank-port))
                   :client-write-cb #'(lambda (afd)
                                        (nvlime-sbcl::client-connect-cb afd dont-close))
                   :client-error-cb #'nvlime-sbcl::socket-error-cb))
