...
(defpackage #:nvlime-usocket
  (:use #:cl
        #:usocket
        #:nvlime-protocol))

(in-package #:nvlime-usocket)
  ...
  (loop
    (handler-case
      (let ((client-socket (socket-accept socket)))
        (swank/backend:spawn
          #'(lambda ()
              (nvlime-control-thread
                client-socket swank-host swank-port))
          :name "Nvlime Control Thread")
        (unless dont-close
          (socket-close socket)
...
(defun nvlime-control-thread (client-socket swank-host swank-port)
  ...
  (let* ((control-thread (swank/backend:current-thread))
         (swank-socket
           (handler-case
             ...
             (t (c)
                ...
                (return-from nvlime-control-thread))))
    ...
    (labels ((read-client-data (buf)
      ...
      (let ((client-read-thread (swank/backend:spawn
                                  #'client-read-loop
                                  :name "Nvlime Client Reader"))
            (swank-read-thread (swank/backend:spawn
                                 #'swank-read-loop
                                 :name "Nvlime SWANK Reader")))
        (loop
...
(defmethod start-server ((backend (eql :usocket)) host port swank-host swank-port dont-close)
  ...
  (let* ((server-socket
    ...
    (values
      (swank/backend:spawn
        #'(lambda ()
            (nvlime-usocket::server-listener server-socket swank-host swank-port dont-close))
        :name (format nil "Nvlime Server Listener ~a ~a" (first local-name) (second local-name)))
      local-name)))
