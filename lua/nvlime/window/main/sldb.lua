...
local sldb = {}
local _2bname_2b = "sldb"
local _2bfiletype_2b = buffer["gen-filetype"](_2bname_2b)
local function gen_sldb_bufname(conn_name, thread)
  return buffer["gen-name"](conn_name, _2bname_2b, thread)
end
...
sldb["on-debug-return"] = function(config)
  local _1_, _2_ = psl_buf["exists?"](gen_sldb_bufname(config["conn-name"], config.thread))
...
sldb.open = function(content, config)
  ...
  bufnr = buffer["create-if-not-exists"](gen_sldb_bufname(config["conn-name"], config.thread), true, _6_)
