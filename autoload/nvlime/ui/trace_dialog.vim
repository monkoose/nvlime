...
function! nvlime#ui#trace_dialog#InitTraceDialogBuf(conn)
  let [_, bufnr] = luaeval('require"nvlime.window.trace".open(_A[1], _A[2])',
        \ [[], { 'conn-name': a:conn.cb_data.name }])
  if !nvlime#ui#NvlimeBufferInitialized(bufnr)
    call setbufvar(bufnr, 'nvlime_conn', a:conn)
    call nvlime#ui#WithBuffer(bufnr, function('s:InitTraceDialogBuffer'))
  endif
  return bufnr
endfunction
...
function! nvlime#ui#trace_dialog#FillTraceDialogBuf(spec_list, trace_count)
  ...
  let b:nvlime_trace_specs_coords = []
  call s:DrawSpecList(a:spec_list, b:nvlime_trace_specs_coords)

  let b:nvlime_trace_entries_header_coords = []
  let cached_entries = get(b:, 'nvlime_trace_cached_entries', {})
  call s:DrawTraceEntryHeader(
        \ a:trace_count, len(cached_entries),
        \ b:nvlime_trace_entries_header_coords)

  setlocal nomodifiable
endfunction

function! nvlime#ui#trace_dialog#RefreshSpecs()
  call b:nvlime_conn.ReportSpecs(
        \ function('s:ReportSpecsComplete', [bufnr('%')]))
endfunction

" nvlime#ui#trace_dialog#Select([action])
function! nvlime#ui#trace_dialog#Select(...)
  ...
  if action == 'button'
    ...
    elseif coord['type'] == 'CLEAR-TRACE-ENTRIES'
      call b:nvlime_conn.ClearTraceTree(
  ...
  elseif coord['type'] == 'TRACE-ENTRY-ARG' || coord['type'] == 'TRACE-ENTRY-RETVAL'
    if action == 'inspect'
      ...
      call b:nvlime_conn.InspectTracePart(
...
function! nvlime#ui#trace_dialog#NextField(forward)
  let coord_groups = [
        \ [get(b:, 'nvlime_trace_specs_line_range', v:null),
        \ sort(copy(get(b:, 'nvlime_trace_specs_coords', [])),
        \ function('nvlime#ui#CoordSorter', [a:forward]))],
        \ [get(b:, 'nvlime_trace_entries_header_line_range', v:null),
        \ sort(copy(get(b:, 'nvlime_trace_entries_header_coords', [])),
        \ function('nvlime#ui#CoordSorter', [a:forward]))],
        \ [get(b:, 'nvlime_trace_entries_line_range', v:null),
        \ sort(copy(get(b:, 'nvlime_trace_entries_coords', [])),
        \ function('nvlime#ui#CoordSorter', [a:forward]))]]
...
" nvlime#ui#trace_dialog#CalcFoldLevel([line_nr])
function! nvlime#ui#trace_dialog#CalcFoldLevel(...)
  ...
  if len(matched) > 0
    let id_width = exists('b:nvlime_trace_max_id') ?
          \ len(string(b:nvlime_trace_max_id)) : 0
...
" nvlime#ui#trace_dialog#BuildFoldText([fold_start])
...
function! s:InitTraceDialogBuffer()
  ...
  setlocal foldtext=nvlime#ui#trace_dialog#BuildFoldText(v:foldstart)
  setlocal foldexpr=nvlime#ui#trace_dialog#CalcFoldLevel(v:lnum)
  setlocal foldmethod=expr
endfunction

function! s:DrawSpecList(spec_list, coords)
  let line_range = get(b:, 'nvlime_trace_specs_line_range', v:null)
  if line_range is v:null
    let first_line = 1
    let last_line = line('$')
  else
  ...
  let spec_list = (a:spec_list is v:null) ? [] : a:spec_list
  ...
  try
    let new_lines_count = nvlime#ui#ReplaceContent(content, first_line, last_line)
  finally
    call setpos('.', old_cur_pos)
  endtry

  let b:nvlime_trace_specs_line_range = [first_line, first_line + new_lines_count - 1]

  let delta = s:CalcLineRangeShift(b:nvlime_trace_specs_line_range, line_range)
  let b:nvlime_trace_entries_header_line_range =
        \ s:ShiftLineRange(
        \ get(b:, 'nvlime_trace_entries_header_line_range', v:null),
        \ delta)
  let b:nvlime_trace_entries_line_range =
        \ s:ShiftLineRange(
        \ get(b:, 'nvlime_trace_entries_line_range', v:null),
        \ delta)
endfunction
...
function! s:DrawTraceEntryHeader(entry_count, cached_entry_count, coords)
  let line_range = get(b:, 'nvlime_trace_entries_header_line_range', v:null)
  if line_range is v:null
    let first_line = line('$')
    let last_line = line('$')
  else
...
function! s:DrawTraceEntries(toplevel, cached_entries, coords, ...)
  ...
  let line_range = get(b:, 'nvlime_trace_entries_line_range', v:null)
  if line_range is v:null
    let first_line = line('$')
    let last_line = line('$')
  else
...
function! s:GetCurCoord()
  ...
  if cur_pos[1] >= b:nvlime_trace_specs_line_range[0] &&
        \ cur_pos[1] <= b:nvlime_trace_specs_line_range[1]
    let line_delta = b:nvlime_trace_specs_line_range[0] - 1
    let shifted_line = cur_pos[1] - line_delta
    for c in b:nvlime_trace_specs_coords
      if nvlime#ui#MatchCoord(c, shifted_line, cur_pos[2])
        return c
      endif
    endfor
  elseif cur_pos[1] >= b:nvlime_trace_entries_header_line_range[0] &&
        \ cur_pos[1] <= b:nvlime_trace_entries_header_line_range[1]
    let line_delta = b:nvlime_trace_entries_header_line_range[0] - 1
    let shifted_line = cur_pos[1] - line_delta
    for c in b:nvlime_trace_entries_header_coords
      if nvlime#ui#MatchCoord(c, shifted_line, cur_pos[2])
        return c
      endif
    endfor
  elseif exists('b:nvlime_trace_entries_line_range') &&
        \ cur_pos[1] >= b:nvlime_trace_entries_line_range[0] &&
        \ cur_pos[1] <= b:nvlime_trace_entries_line_range[1]
    let line_delta = b:nvlime_trace_entries_line_range[0] - 1
    let shifted_line = cur_pos[1] - line_delta
    for c in b:nvlime_trace_entries_coords
      if nvlime#ui#MatchCoord(c, shifted_line, cur_pos[2])
        return c
      endif
    endfor
...
function! s:ReportSpecsComplete(trace_buf, conn, result)
  ...
  call nvlime#ui#WithBuffer(a:trace_buf,
        \ function('s:DrawSpecList', [a:result, coords]))
  call setbufvar(a:trace_buf, '&modifiable', 0)
  call setbufvar(a:trace_buf, 'nvlime_trace_specs_coords', coords)
endfunction

function! s:ReportTotalComplete(trace_buf, conn, result)
  let cached_entries =
        \ getbufvar(a:trace_buf, 'nvlime_trace_cached_entries', {})
  ...
  call setbufvar(a:trace_buf, '&modifiable', 0)
  call setbufvar(a:trace_buf, 'nvlime_trace_entries_header_coords', coords)
endfunction
...
function! s:ReportPartialTreeComplete(trace_buf, fetch_all, conn, result)
  ...
  let entry_list = (entry_list is v:null) ? [] : entry_list

  let cached_entries =
        \ getbufvar(a:trace_buf, 'nvlime_trace_cached_entries', {})
  let toplevel_entries =
        \ getbufvar(a:trace_buf, 'nvlime_trace_toplevel_entries', [])
  let max_id = getbufvar(a:trace_buf, 'nvlime_trace_max_id', 0)
  ...
  for t_entry in entry_list
    ...
    let parent_obj = (parent is v:null) ?
          \ v:null : get(cached_entries, parent, v:null)
    if parent_obj is v:null
  ...
  call setbufvar(a:trace_buf, 'nvlime_trace_cached_entries', cached_entries)
  call setbufvar(a:trace_buf, 'nvlime_trace_toplevel_entries', toplevel_entries)
  call setbufvar(a:trace_buf, 'nvlime_trace_max_id', max_id)
  ...
  if a:fetch_all && remaining > 0
    ...
    call setbufvar(a:trace_buf, '&modifiable', 0)
    call setbufvar(a:trace_buf, 'nvlime_trace_entries_coords', coords)
  endif
endfunction

function! s:ClearTraceTreeComplete(trace_buf, conn, result)
  call nvlime#ui#WithBuffer(a:trace_buf, function('s:ResetTraceEntries'))
  call b:nvlime_conn.ReportTotal(
        \ function('s:ReportTotalComplete', [a:trace_buf]))
endfunction

function! s:ResetTraceEntries()
  silent! unlet b:nvlime_trace_fetch_key
  silent! unlet b:nvlime_trace_cached_entries
  silent! unlet b:nvlime_trace_toplevel_entries
  silent! unlet b:nvlime_trace_max_id
  silent! unlet b:nvlime_trace_entries_coords

  let line_range = get(b:, 'nvlime_trace_entries_line_range', v:null)
  silent! unlet b:nvlime_trace_entries_line_range

  if line_range isnot v:null
    let [first_line, last_line] = line_range
    setlocal modifiable
...
function! s:GetFetchKey()
  if !exists('b:nvlime_trace_fetch_key')
    let fetch_key = get(s:, 'next_fetch_key', 0)
    let s:next_fetch_key = fetch_key + 1
    if s:next_fetch_key > 65535
      let s:next_fetch_key = 0
    endif
    let b:nvlime_trace_fetch_key = getpid() . '_' . fetch_key
  endif
  return b:nvlime_trace_fetch_key
endfunction

function! s:ArgListToDict(arg_list)
  let arg_list = (a:arg_list is v:null) ? [] : a:arg_list
  let args = {}
  for r in arg_list
    let args[r[0]] = r[1]
  endfor
